...
#            is not found in the DeviceSubscription table and device_id is not
#            found in the DeviceNetworkStatus table then,
#            'Device Not Found' error is returned as response.
            if not status_res['Items'] and not network_res:
                devices.append(
                    create_devices_layer(None, device_id, code=DEVICE_NOT_FOUND
                                         ))
            else:
...
#                latest or not by checking with DeviceSubscription table data
                ...
                parsed_res = parse(device_log, verified_res)
                parsed_res.append(
                    create_feature_format(SUCCESS, 'Online_Offline',
                                          network_res['status'],
                                          network_res['timestamp'])
                )
                features = create_features_layer(parsed_res)
                devices.append(create_devices_layer(features, device_id))
        response = {
            "statusCode": 200,
            "body": json.dumps(create_odessa_layer(devices))
        }
        return response
        ...
        response = {
            "statusCode": 200,
            "body": json.dumps(create_odessa_layer([], code=BAD_REQUEST))
        }
        return response
    except ServiceIdError as e:
        logger.error(e)
        logger.warning(
        ...
        response = {
            "statusCode": 200,
            "body": json.dumps(
                create_odessa_layer([], code=BOC_DB_CONNECTION_ERROR))
        }
        return response
