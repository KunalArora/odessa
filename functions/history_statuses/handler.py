...
def get_history_statuses(event, context):
    ...
    if reporting_id is not None:
        if (not isinstance(reporting_id, str) or reporting_id == ""):
            logger.warning(
                f"BadRequest on handler:get_history_statuses, "
                f"Reason: Parameter 'reporting_id' = {reporting_id} "
                f"has incorrect value")
            return history_statuses_response(
                odessa_response_codes.BAD_REQUEST, reporting_id,
                message=f"Parameter 'reporting_id' has "
                f"incorrect value: '{reporting_id}'")
